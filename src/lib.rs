        ...
        p.add_function("all", |c| {
            ...
            if let (ExprValue::Array(a), Some(predicate)) = (&c.args[0], c.predicate) {
                for value in a {
                    ...
                    if let ExprValue::Bool(false) = c.parser.run(predicate.clone(), &ctx)? {
...
bar`"#,
    ...
    fn array_functions() -> Result<()> {
        ...
        test!(r#"any([1, 2, 3], {# > 3})"#, "false");
        test!(r#"one([1, 2, 3], {# > 2})"#, "true");
        test!(r#"one([1, 2, 3], {# > 1})"#, "false");
        test!(r#"none([1, 2, 3], {# > 3})"#, "true");
        test!(r#"none([1, 2, 3], {# > 2})"#, "false");
        test!(r#"map([1, 2, 3], {# * 2})"#, "[2, 4, 6]");
        test!(r#"filter([1, 2, 3], {# % 2 == 0})"#, "[2]");
        test!(r#"find([1, 2, 3], {# % 2 == 0})"#, "2");
